...
NS_LOG_COMPONENT_DEFINE ("APCSMAEXT3");
...
int main(int argc, char *argv[])
{
    ...
    if (verbose)
    {
      ...
      LogComponentEnable ("APCSMAEXT3", LOG_LEVEL_INFO);
    }
    ...
    for(i=0; i<2; i++){
        Ptr<Ipv4StaticRouting> ptr_staticRouting = staticRoutingHelper.GetStaticRouting(csmanc.Get(i)->GetObject<Ipv4>());
        ptr_staticRouting->AddHostRouteTo(Ipv4Address("192.168.3.0"), 2, i);
    }
    NS_LOG_INFO("Set static routing for network2's ap1 and ap2");
    for(i=2; i<4; i++){
        Ptr<Ipv4StaticRouting> ptr_staticRouting = staticRoutingHelper.GetStaticRouting(csmanc.Get(i)->GetObject<Ipv4>());
        ptr_staticRouting->AddHostRouteTo(Ipv4Address("192.168.1.0"), 2, i);
    }
    ...
    UdpEchoClientHelper echoClient (wn2intf.GetAddress (2), 9);
    ...
    if(tracing == true){
        csma.EnablePcapAll("pcap/apcsmaext3");
        phy.EnablePcapAll("pcap/apcsmaext3");
    }

    AnimationInterface anim("xml/apcsmaext3");
