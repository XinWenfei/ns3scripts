...
NS_LOG_COMPONENT_DEFINE ("apcsmaext5");
...
int main(int argc, char *argv[])
{
    ...
    if (verbose)
    {
      ...
      LogComponentEnable ("apcsmaext5", LOG_LEVEL_INFO);
    }
    ...
    */
    ...
    if(tracing == true){
        csma.EnablePcapAll("pcap/apcsmaext5");
        phy.EnablePcapAll("pcap/apcsmaext5");
    }

    NS_LOG_INFO("Establish global routes");
    Ipv4GlobalRoutingHelper::PopulateRoutingTables ();

    Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("route/apcsmaext5.routes", std::ios::out);
    Ipv4GlobalRoutingHelper::PrintRoutingTableAllAt (Seconds (8), routingStream);


    AnimationInterface anim("xml/apcsmaext5");
