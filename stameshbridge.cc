...
int main(int argc, char* argv[])
{
   ...
   if(pcap)
    ...
    Ipv4InterfaceContainer bridgeIntf;
    bridgeIntf.Add( address.Assign(bridgeNetDevs1) );

    Ipv4InterfaceContainer meshIntf;
    for(uint16_t i = 1; i < rowNodes*colNodes-1; i++){
        meshIntf.Add(address.Assign(meshDevices.Get(i)));
    }

    bridgeIntf.Add( address.Assign(bridgeNetDevs2) );

    Ipv4InterfaceContainer staIntf = address.Assign(staDevices);

    Ptr<Ipv4> ptr_ipv4bridge1 = meshNC.Get(0)->GetObject<Ipv4>();
    Ptr<Ipv4> ptr_ipv4bridge2 = meshNC.Get(rowNodes*colNodes - 1)->GetObject<Ipv4>();

    Ipv4Address ipbridge1  = ptr_ipv4bridge1->GetAddress(1, 0).GetLocal();
    Ipv4Address ipbridge2  = ptr_ipv4bridge2->GetAddress(1, 0).GetLocal();

    std::cout<<"bridge netdevice one ip: "<<ipbridge1<<std::endl;
    std::cout<<"bridge netdevice two ip: "<<ipbridge2<<std::endl;
    ...
    UdpEchoClientHelper echoClient(staIntf.GetAddress(1), 9);
