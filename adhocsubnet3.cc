...
//	cout << "mobility model is:" << ptr_mm << endl;
    ...
    AnimationInterface anim("xml/adhocsubnet3.xml");
    ...
    anim.SetConstantPosition(nc.Get(4), 50,20);
    anim.SetConstantPosition(nc.Get(nWifi-1), 60,10);
    anim.UpdateNodeColor(nc.Get(0), 0, 255, 0);
    anim.UpdateNodeColor(nc.Get(nWifi-1), 0, 0, 255);
    ...
    if(tracing){
		phy.EnablePcapAll("pcap/adhocsubnet3");
    }
