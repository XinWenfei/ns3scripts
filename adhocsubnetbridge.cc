...
//	cout << "mobility model is:" << ptr_mm << endl;
    ...
    BridgeHelper bridge;
    NetDeviceContainer bridgedc;
    bridgedc = bridge.Install(nc.Get(2), NetDeviceContainer(netdev.Get(2), netdev.Get(nWifi-1)));
    ...
    Ipv4InterfaceContainer bridgeIntfc;
    intfc.Add( address.Assign(netdev.Get(0)));
    intfc.Add( address.Assign(netdev.Get(1)));
    bridgeIntfc.Add( address.Assign(bridgedc));
    //for(uint16_t i = 0; i<3; i++)
    //    intfc.Add( address.Assign(netdev.Get(i)));
    ...
    AnimationInterface anim("xml/adhocsubnet4.xml");
    ...
    if(tracing){
		phy.EnablePcapAll("pcap/adhocsubnet4");
    }
